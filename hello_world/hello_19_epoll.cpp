/*
    ...
    of us rebuilding a vector each time. That's what makes it scale past poll()'s
    O(n) per-call scan to thousands of mostly-idle connections.
*/
...
void cleanup_client(int epfd, int client_fd, std::unordered_map<int, PendingWrite>& pending_writes,
                     std::unordered_map<int, std::string>& pending_reads) {
    epoll_ctl(epfd, EPOLL_CTL_DEL, client_fd, nullptr);  // deregister before close(), not after
    close(client_fd);
    pending_writes.erase(client_fd);
    pending_reads.erase(client_fd);
}
...
int main() {
    ...
    // epoll's watch list lives in the kernel (epfd), not in a vector we rebuild
    // every iteration; epoll_ctl() adds/modifies/removes one fd at a time.
    ...
    std::vector<epoll_event> events(64);  // scratch buffer epoll_wait() fills with only the ready fds
    std::unordered_map<int, PendingWrite> pending_writes;
    std::unordered_map<int, std::string> pending_reads;
