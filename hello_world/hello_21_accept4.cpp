/*
    ...
    fd exists but is still blocking.
*/

#define _GNU_SOURCE  // glibc hides accept4()/SOCK_NONBLOCK behind this with -std=c++20

#include <iostream>
#include <sys/socket.h>
#include <netinet/in.h>
#include <unistd.h>
#include <fcntl.h>  // fcntl(), O_NONBLOCK (still needed for server_fd)
...
int main() {
    ...
    while (true) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (fd == server_fd) {
                while (true) {
                    // SOCK_NONBLOCK folds the fcntl(O_NONBLOCK) call every prior
                    // step made right after accept() into the accept itself.
