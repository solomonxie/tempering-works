/*
    $ clang++ -std=c++20 -Wall -Wextra -g hello_world/hello_09_select.cpp -o /tmp/hello_09_select && /tmp/hello_09_select
    then:
    ...
    Step 1: select() watches server_fd for readability — is a new connection waiting?
    — instead of accept()-ing blindly. This is the oldest of the readiness APIs
    (predates poll() and epoll()); hello_10_poll.cpp does the exact same thing with
    poll() instead, to show how the syntax differs.
*/
...
int main() {
    ...
    while (true) {
        // Step 1: ask select() to watch server socket for readability (is data/connection ready?).
        // Unlike pollfd (one struct per fd, reused across calls), fd_set is a fixed-size
        // bitmask capped at FD_SETSIZE (usually 1024) fds, and select() overwrites it in
        // place with only the ready fds — so it has to be rebuilt with FD_ZERO/FD_SET
        // every single loop iteration, not just populated once.
        fd_set read_fds;
        FD_ZERO(&read_fds);
        FD_SET(server_fd, &read_fds);

        std::cout << "Waiting for a client...\n";
        // First argument is "highest fd number + 1", not a count of fds — the kernel
        // scans bit-by-bit from 0 up to that number, which is why select() doesn't
        // scale to large fd numbers the way poll()/epoll() do.
        int select_result = select(server_fd + 1, &read_fds, nullptr, nullptr, nullptr);
        std::cout << "select() returned: " << select_result
                   << ", server_fd ready: " << FD_ISSET(server_fd, &read_fds) << std::endl;
